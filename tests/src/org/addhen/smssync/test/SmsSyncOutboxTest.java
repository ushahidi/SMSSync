...
/**
 * Testcase for testing submission of pending messages when SMS fails to submit to the configured 
 * URL.
 * 
 * @author eyedol
 *
		...
		if (cursor.moveToFirst()) {
			...
			do {
				...
				if( Util.postToAWebService(messagesFrom, messagesBody,mSmsSyncOutbox) ) {
					...
					status = 0; //successfully posted messages to the web service.
				} else {
					status = 1; // failed to post the messages to the web service.
				}
